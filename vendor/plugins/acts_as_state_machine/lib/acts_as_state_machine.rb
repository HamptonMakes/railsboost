  ...
  module Acts                        #:nodoc:
    module StateMachine              #:nodoc:
      ...
      module InstanceMethods
        ...
        def current_state
          (self.send(self.class.state_column) || initial_state).to_sym
        end

        # Returns the current state the object is in, as a human-readable string.
        def status
        ...
        def transition(options={})
          to, from = options[:to], options[:from]
          if valid_transition?(to, from)
            if update_attribute(:state, to.to_s)
              reload
              true
            else
              false
        ...
        def valid_transition?(to, from=nil)
          if current_state == to.to_sym
            false 
          elsif from && !Array(from).include?(current_state)
            false
          else
            true
          end
