...
class ActsAsStateMachineTest < Test::Unit::TestCase
  ...
  def test_find_all_in_state
    3.times { Factory.create_conversation!(:subject => 'test', :state_machine => 'read') }
    assert_equal 3, Conversation.find_in_state(:all, :read).size
  end
  
  def test_find_all_in_state_with_conditions
    Factory.create_conversation!(:subject => 'test', :state_machine => 'read')
    assert_equal 1, Conversation.find_in_state(:all, :read, :conditions => {:subject => 'test'}).size
  end
  
  def test_find_first_in_state_with_conditions
    Factory.create_conversation!(:subject => 'test', :state_machine => 'read')
    assert Conversation.find_in_state(:first, :read, :conditions => {:subject => 'test'})
  end
  ...
  def test_count_in_state_with_conditions
    Factory.create_conversation!(:subject => 'test', :state_machine => 'read')
    cnt0 = Conversation.count(:conditions => ['state_machine = ? AND subject = ?', 'read', 'test'])
    cnt  = Conversation.count_in_state(:read, :conditions => ['subject = ?', 'test'])
